...
@Component({
  ...
  standalone: true,
  imports: [CommonModule,ReactiveFormsModule,MatTableModule,MatCardModule,NgxSpinnerModule],
...
export class UserKYCComponent extends BasecomponentComponent implements OnInit {
  ...
  displayedColumns: string[] = ['UserKYCID', 'KycTypeName', 'DocumentNo', 'FileUrl', 'CreatedOn', 'CreatedBy',];
  dataSource = this.Modeldata;
  addnew:boolean=false;


  constructor(private ngxs:NgxSpinnerService, private fb: FormBuilder,private hhtp:HttpClient, private mstdataservice: MasterDataService, private frmBuilder: FormBuilder, private users: UserMasterService,toster:ToastrService) {
    super(toster)
    this.createForm();
  }
  createForm(){
